...
class TopIO extends Bundle {
        // Output ports
        // - RVFI pins
        val RegFDInst        : UInt = Output(UInt(32.W))
...
class Top extends Module {
        ...
        val io         : Top_IO      = IO(new TopIO)
        val C          : Consts      = new Consts
        val PC         : PC          = Module(new PC(C.XLEN, C.MDEPTH))
        val InstMem    : InstMem     = Module(new InstMem(C.XLEN, C.MDEPTH))
        val RegFD      : RegFD       = Module(new RegFD(C.XLEN))
        ...
        Seq(
                ...
                // Instruction Memory
                InstMem.io.instAddr, InstMem.io.stallEn,
                ...
                // Instruction Memory
                PC.io.instAddr, PC.io.stallEn,

                // RegFD
                PC.io.pcOut, PC.io.instOut, PC.io.pc4,
